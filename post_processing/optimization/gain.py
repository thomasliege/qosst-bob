...
def optimize_gain(gain, data_alice_normalized, Va, T, xi, vel, eta, beta):
    # Initialize 2D array for key rates
    keyrate_ppA_map = np.zeros((len(gain), len(gain)))
    
    # 2D scan over g1 and g2 with progress bar
    total_iterations = len(gain) * len(gain)
    with tqdm(total=total_iterations, desc="Scanning gain values", unit="iter") as pbar:
        for i, g1 in enumerate(gain):
            for j, g2 in enumerate(gain):
                post_selection_alice = alice_post_selection(
                    data_alice_normalized, np.array([g1,g2]), np.array([Va, Va])
                )
                ...
                I_AB_ppA, I_AB_raw = test_I_AB_ppA(np.array([Vmod_tilde_x, Vmod_tilde_p]), T, xi, vel, eta, P_A_x, P_A_p)
                I_E_ppA = test_IE_ppA(np.array([Vmod_tilde_x, Vmod_tilde_p]), T, xi, eta, vel, P_A_x, P_A_p)
